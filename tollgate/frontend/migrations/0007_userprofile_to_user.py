...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		# Port forwards
		for fwd in orm.IP4PortForward.objects.all():
			fwd.creator_u = fwd.creator.user
			fwd.save()
			
		# Event attendance
		for att in orm.EventAttendance.objects.all():
			att.user = att.user_profile.user
			att.registrar = None if att.registered_by is None else att.registered_by.user
			att.save()
		
		# Quota reset event
		for qr in orm.QuotaResetEvent.objects.all():
			qr.user = qr.performer.user
			qr.save()
		
		# NetworkHostOwnerChangeEvent
		for nhoce in orm.NetworkHostOwnerChangeEvent.objects.all():
			nhoce.old_owner_u = None if nhoce.old_owner is None else nhoce.old_owner.user
			nhoce.new_owner_u = None if nhoce.new_owner is None else nhoce.new_owner.user
			nhoce.save()
		
		# Network hosts
		for host in orm.NetworkHost.objects.all():
			host.owner = None if host.user_profile is None else host.user_profile.user
			host.save()
